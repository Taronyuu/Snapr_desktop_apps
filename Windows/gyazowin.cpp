...
HINSTANCE hInst;
TCHAR *szTitle			= _T("Snapr.pw");
TCHAR *szWindowClass	= _T("SnaprWIN");
TCHAR *szWindowClassL	= _T("SnaprWINNL");
HWND hLayerWnd;
...
BOOL uploadFile(HWND hwnd, LPCTSTR fileName)
{
	const TCHAR* UPLOAD_SERVER	= _T("snapr.pw");
	const TCHAR* UPLOAD_PATH	= _T("/snapr.php");
					...
					(DWORD) oMsg.length()))
	{
		...
		if( _ttoi(resCode) != 200 ) {
			...
			MessageBox(hwnd, _T("Something went wrong while uploading. Please try again."),
				szTitle, MB_ICONERROR | MB_OK);
		} else {
